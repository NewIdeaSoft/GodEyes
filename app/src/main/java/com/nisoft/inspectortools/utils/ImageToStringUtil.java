...
public class ImageToStringUtil {
    private static final String TESS_BASE_PATH = Environment.getExternalStorageDirectory().getAbsolutePath()
            +"/tesseract/";
...
//        Bitmap data = handleBlackWitheBitmap(bitmap);
    ...
    public static Bitmap handleBlackWitheBitmap(Bitmap bitmap) {
        return bitmap;
    }
    public static void resourseToFile(Context context){
        if (!new File(TESS_BASE_PATH + DEFAULT_LANGUAGE).exists())
            //推送字库到SD卡
